...
@Client.on_message(
    command(["join", f"join@{BOT_USERNAME}"]) & other_filters
)
...
async def join_chat(c: Client, m: Message):
    ...
    try:
        ...
        return await user.send_message(chat_id, "انا موجود يا عيني")

    
@Client.on_message(filters.command("جاك ادخل", [".", ""]) & ~filters.edited)
@authorized_users_only
...
@Client.on_message(filters.command("جاك اطلع", [".", ""]) & ~filters.edited)
@authorized_users_only
async def leave_chat(_, m: Message):
    chat_id = m.chat.id
    try:
