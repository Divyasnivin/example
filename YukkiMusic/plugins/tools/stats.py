...
**عدد الاستفسارات :** {total_queries} 
** عدد الحسابات المساعدة:** {assistant}
**الخروج التلقائي للحساب المساعد  :** {ass}
**مدة وضع التنظيف :** {cm} Mins

** حد الدقائق الممكن تشغيلها :** {play_duration} Mins
**حد الدقائق لتحميل الاغنية  :** {song} دقائق
**   العدد المحدد لقائمة تشغيل البوت :** {playlist_limit}
**العدد المحدد لقائمة التشغيل  :** {fetch_playlist}"""
...
**اصدار البايثون  :** {pyver.split()[0]}
** نسخة البايروجرام :** {pyrover}
**تي جي اصدار :** {pytgver}

** المتوفر للتخزين:** {total[:4]} GiB
**التخزين المستخدم :** {used[:4]} GiB
**باقي التخزين :** {free[:4]} GiB

**عدد المجموعات :** {served_chats} 
** عدد المشتركين:** {served_users} 
**المحظرين :** {blocked} 
**المطورين :** {sudoers} 

**وقت التشغيل :** {mongouptime[:4]} Days
** إجمالي حجم قاعدة البيانات :** {datasize[:6]} Mb
**عدد سعة التخزين قاعدة البيانات  :** {storage} Mb
** إجمالي مجموعات قاعدة البيانات :** {collections}
** مجموعة مفاتيح قاعدة البيانات :** {objects}
**عدد استعلامات قاعدة البيانات  :** `{query}`
**إجمالي عدد استعلامات البوت  :** `{total_queries} `
    """
    med = InputMediaPhoto(media=config.STATS_IMG_URL, caption=text)
    try:
        await CallbackQuery.edit_message_media(
